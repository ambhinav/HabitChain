...
contract Habit {
    ...
    struct habit {
        ...
        uint256 num_users;
        uint256 habit_type;
        uint256 wake_up_time; //set to 8AM
        uint256 run_distance; //set to 5KM
        bool is_deleted;
    }
    ...
    event TickUserList(uint256 habit_id, address user_addr, uint256 date_num);
    ...
    modifier is_not_loser(uint256 habit_id, address user_addr) {
        require(
            !habits[habit_id].users[user_addr].is_loser,
            "User has lost, no need to check on him."
        );
        _;
    }
     ...
     */
    function create_habit(uint256 start_time_, uint256 habit_type_)
        public
        payable
    {
        require(
            block.timestamp < start_time_,
            "Start time must be in the future"
        );
        require(
            habit_type_ == 0 || habit_type_ == 1,
            "Habit type must be either id 0 or 1"
        );
        ...
        join_habit(new_habit_id);
        emit CreateHabit(msg.sender, new_habit_id, start_time_, habit_type_);
        emit JoinHabit(msg.sender, new_habit_id, msg.value);
    }
     ...
     */
    function join_habit(uint256 habit_id)
        public
        payable
        is_valid_id(habit_id)
        is_not_deleted(habit_id)
    {
        require(msg.value > 0, "Pledge amount must be more than 0");
        require(
            habits[habit_id].users[msg.sender].addr == address(0),
        ...
        habits[habit_id].user_addresses[habits[habit_id].num_users] = msg
            .sender;
     ...
     *
     */
    function tick_user_list(
        uint256 habit_id,
        address user_addr,
        uint256 date_num
    ) public is_valid_id(habit_id) is_not_deleted(habit_id) {
     ...
     */
    function end_habit(uint256 habit_id)
        public
        is_valid_id(habit_id)
        is_not_deleted(habit_id)
    ...
    {
        require(
            block.timestamp > habits[habit_id].end_time,
        ...
        for (uint256 i = 0; i < habits[habit_id].num_users; i++) {
        ...
        for (uint256 j = 0; j < num_winners; j++) {
            address payable recipient = address(uint160(winners[j]));
            uint256 to_receive = habits[habit_id].users[winners[j]].pledge_amt;
     ...
     */
    function verify(uint256 habit_id, address user_addr)
        private
        is_valid_id(habit_id)
        is_not_deleted(habit_id)
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < curr_user.check_list.length; i++) {
    ...
    function get_start_time(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (uint256)
    ...
    function get_end_time(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (uint256)
    ...
    function get_habit_type(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (uint256)
    ...
    function get_owner(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (address)
    ...
    function get_pool(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (uint256)
    ...
    function get_num_users(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (uint256)
    ...
    function get_run_distance(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (uint256)
    ...
    function get_wake_up_time(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (uint256)
    ...
    function is_user_joined_habit(uint256 habit_id, address user_)
        public
        view
        is_valid_id(habit_id)
        returns (bool)
    ...
    function is_user_a_loser(uint256 habit_id, address user_)
        public
        view
        is_valid_id(habit_id)
        returns (bool)
    ...
    function get_user_check_list(uint256 habit_id, address user_)
        public
        view
        is_valid_id(habit_id)
    ...
    function is_habit_deleted(uint256 habit_id)
        public
        view
        is_valid_id(habit_id)
        returns (bool)
